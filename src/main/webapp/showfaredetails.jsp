...
<body>
	...
	<div class="col-md-8" style="margin: auto;">
		...
		<div class="shadow-lg p-3 mb-5 bg-white rounded" style="border: none;">
			<table class="table col-md-12" style="margin:;">
				...
				<tbody>					
				     <c:forEach var="fareDetails" items="${fareDetailsList}">
				      <tr>
				         <td id="flightNo">${fareDetails.getFlightNo()}</td>
				         <td>${fareDetails.getAirline()}</td>
				         <td>${fareDetails.getFlightClass()}</td>
				         <td>${fareDetails.getFare()}</td>
				         <td>
				         	<form action="BookingDetailsServlet" method="Post">
				         	 	<input type="hidden" name="flightNo" value="${fareDetails.getFlightNo()}">
				         	 	<input type="hidden" name="airline" value="${fareDetails.getAirline()}">
				         	 	<input type="hidden" name="class" value="${fareDetails.getFlightClass()}">
				         	 	<input type="hidden" name="fare" value="${fareDetails.getFare()}"> 
	         	            	<button type="submit" class="btn btn-primary" style="width: 110px;" onclick="redirectToPage()">Book Now</button>
				         	</form>
				         </td>
